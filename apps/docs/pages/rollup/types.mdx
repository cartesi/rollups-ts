...
Everything the package exports besides the [`Rollup`](/rollup/Rollup) class (the `RollupError` class is documented under [Errors](#errors)). All types are importable with `import type`:
...
Three "like" types describe what the binding accepts and converts automatically — see [byte and number conventions](/rollup/emitting-outputs#byte-and-number-conventions). Hex strings are `Hex` (`` `0x${string}` ``), structurally identical to viem's `Hex`/`Address`:
...
- `Voucher` — argument of [`emitVoucher`](/rollup/emitVoucher): `destination` (`AddressLike`), optional `value` (`U256Like`, default `0n`) and `payload` (`BytesLike`, default empty).
- `DelegateCallVoucher` — argument of [`emitDelegateCallVoucher`](/rollup/emitDelegateCallVoucher): `destination` (`AddressLike`) and optional `payload` (`BytesLike`). No `value` — `DELEGATECALL` cannot transfer ether.
...
[`finish`](/rollup/finish) returns `RollupRequest`, the discriminated union of:
...
`GioResponse` is returned by [`gio`](/rollup/gio): `{ responseCode: number, responseData: Buffer }`.

## Constants

```ts twoslash
import { ADDRESS_LENGTH, U256_LENGTH } from "@cartesi/rollup";
