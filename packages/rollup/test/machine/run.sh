...
#
# Requires: docker (with riscv64 emulation), cartesi-machine, node, pnpm, wget.
#
...
#     cartesi/machine-emulator:0.21.0 cartesi-machine" test/machine/run.sh
...
# 3. build the TypeScript half, pack the package and stage the dapp directory
# (prebuild rides along in the tarball; --ignore-scripts skips node-gyp-build's
# host-side install)
if [ -z "${SKIP_ROOTFS:-}" ]; then
    ...
    docker build --platform linux/riscv64 \
        ...
        -t cartesi-rollup-machine-rootfs "$CONTEXT"
    cid=$(docker create --platform linux/riscv64 cartesi-rollup-machine-rootfs)
...
rm -f "$WORK"/input-*-output-*.bin "$WORK"/input-*-report-*.bin \
    "$WORK"/query-report-*.bin "$WORK"/input-*-outputs-merkle-root.bin
# --no-revert: snapshotting is only needed to
# revert rejected advances and inspects, and this test accepts everything.
# The output/merkle-root proof patterns are set to the empty string to disable
# them: this test verifies the output bytes, not their proofs, and otherwise
# 0.21 would write them to the package root (the cwd). The outputs merkle root
# itself is still checked against the guest's (check_outputs_merkle_root
# defaults to true).
$CARTESI_MACHINE \
    --no-revert \
    --ram-image="$KERNEL" \
    --ram-length=512Mi \
    --flash-drive=label:root,data_filename:"$WORK/rootfs.ext2" \
    --cmio-advance-state=input:"$WORK/input-%i.bin",input_index_begin:0,input_index_end:2,output:"$WORK/input-%i-output-%o.bin",report:"$WORK/input-%i-report-%o.bin",outputs_merkle_root:"$WORK/input-%i-outputs-merkle-root.bin",output_proof:,outputs_merkle_root_proof: \
