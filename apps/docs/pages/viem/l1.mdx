...
:::tip
[Wagmi](https://wagmi.sh) users can use the generated actions and React hooks of `@cartesi/wagmi`, or generate code tailored to their own project using the [`@cartesi/wagmi-plugin`](/wagmi-plugin) plugin for `@wagmi/cli`.
:::
...
import { createWalletClient } from "viem";
import { etherPortalAbi, etherPortalAddress } from "@cartesi/viem/abi";
import { privateKeyToAccount } from "viem/accounts";
import { mainnet } from "viem/chains";
...
The `toEVM` utility converts an `Output` returned by the Cartesi node API into the arguments expected by `IApplication.executeOutput`.

```ts twoslash
import { http } from "viem";
import { createWalletClient } from "viem";
import { createCartesiPublicClient, toEVM } from "@cartesi/viem";
import { iApplicationAbi } from "@cartesi/viem/abi";
import { privateKeyToAccount } from "viem/accounts";
import { mainnet } from "viem/chains";
...
`toEVM` throws if the output has no proof yet (`outputHashesSiblings`).
