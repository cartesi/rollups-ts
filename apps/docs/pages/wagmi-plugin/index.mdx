...
Combined with other Wagmi CLI plugins, it can generate fully typed [viem contract actions](https://wagmi.sh/cli/api/plugins/actions) or [React hooks](https://wagmi.sh/cli/api/plugins/react) for the Cartesi contracts. It is the tool used by `@cartesi/viem` and `@cartesi/react` themselves for their code generation.
