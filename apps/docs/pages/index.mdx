...
Node.js bindings for [libcmt](https://github.com/cartesi/machine-guest-tools/tree/main/sys-utils/libcmt), the Cartesi Machine guest rollup library, so applications running *inside* a Cartesi Machine can handle advance and inspect requests and emit vouchers, notices, reports and exceptions — without going through the rollup HTTP server.
