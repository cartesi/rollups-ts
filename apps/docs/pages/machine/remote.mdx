...
Here is a modified version of the [local](/machine/local) example that creates a remote machine instead.
As simple as that.

```ts twoslash
// @noErrors
import { BreakReason, create } from "@cartesi/machine"; // [!code --]
import { BreakReason, spawn } from "@cartesi/machine"; // [!code ++]
...
The returned `machine` object is now a [RemoteCartesiMachine](/machine/RemoteCartesiMachine), which extends the [CartesiMachine](/machine/CartesiMachine) interface, thus providing all its methods, and a few additional ones.
...
The [RemoteCartesiMachine](/machine/RemoteCartesiMachine) API provides a `fork` method that allows you to create a "copy" of an existing machine, execute it, and throw it away (or not).
