...
While handling a request your application can emit four kinds of outputs. They differ in what they can do and in what can be _proven_ about them on-chain:

| Output                                    | Provable | Purpose                                                        |
| ----------------------------------------- | -------- | -------------------------------------------------------------- |
| [Voucher](/rollup/emitVoucher)            | yes      | Execute a transaction on-chain (transfer assets, call a contract) |
| [Notice](/rollup/emitNotice)              | yes      | Attest a fact about the application state                      |
| [Report](/rollup/emitReport)              | no       | Logs, diagnostics, inspect responses                           |
| [Exception](/rollup/emitException)        | no       | Signal that the request could not be processed                 |
...
There is also a [delegate-call variant](/rollup/emitDelegateCallVoucher) that executes the target's code in the application contract's own storage context — an advanced tool for upgrade-style patterns.
...
Reports are for everything that doesn't need a proof: log lines, error messages, and answers to [inspect queries](/rollup/handling-requests#inspect-requests). They cost nothing on-chain and cannot be verified:
...
An exception tells the outside world the request could not be processed at all — a structural failure rather than a business-rule rejection. Prefer returning `false` from a handler (or throwing, which [run](/rollup/run) turns into a report + rejection); reach for [`emitException`](/rollup/emitException) only when the application cannot continue in a meaningful way.
...
Hex strings are typed as `` `0x${string}` `` ([`Hex`](/rollup/types#input-types)), so values from [viem](https://viem.sh) — `Hex` from `encodeFunctionData`, `Address`, `bigint` from `parseEther` — pass in directly, and request addresses pass back into viem without casts.
