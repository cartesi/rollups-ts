...
Those are regular contract interactions, best served by viem's own [contract actions](https://viem.sh/docs/contract/writeContract) combined with the typed ABIs and deployment addresses exported by `@cartesi/client/abi`:
...
```typescript
import { createCartesiPublicClient, getInputsAdded } from "@cartesi/client";
import { inputBoxAbi, inputBoxAddress } from "@cartesi/client/abi";
