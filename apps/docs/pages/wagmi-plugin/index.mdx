...
Set its [`prt` option](/wagmi-plugin/prt) to generate the contracts of a PRT (Permissionless Refereed Tournaments) deployment instead — the same rollups contracts, at the same addresses, plus the consensus and tournament contracts a [dave](https://github.com/cartesi/dave) release adds.
...
| `prt`         | `boolean \| PrtOptions`                             | Generate a [PRT deployment's contracts](/wagmi-plugin/prt) instead: the same rollups contracts plus the consensus and tournament ones. Defaults to `false`. |
