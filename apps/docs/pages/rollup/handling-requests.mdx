...
- **Advance** — an input submitted on-chain. It can change application state and produce provable outputs ([vouchers](/rollup/emitVoucher) and [notices](/rollup/emitNotice)).
- **Inspect** — an off-chain, read-only query. It can only answer with [reports](/rollup/emitReport).

This page shows the request lifecycle; [Emitting Outputs](/rollup/emitting-outputs) covers what you can produce while handling them.

## The request cycle

Processing follows a strict cycle, inherited from how the machine and the blockchain interact:

1. The application calls [`finish`](/rollup/finish), telling the machine whether it **accepts or rejects** the previous request.
...
[`run`](/rollup/run) implements this cycle; your handlers plug into step 3.
...
Remember that the machine is paused _between_ requests, not within them: timers and I/O work normally while your handler runs, but nothing executes while the application waits for the next request.

## Driving the loop yourself

`run` is a thin convenience. If you prefer explicit control — custom dispatch, graceful shutdown, request batching — call [`finish`](/rollup/finish) directly:
